...
| `/products` | Refresh the complete PIN-specific catalog; full-name text and full-width select/unselect buttons, with selected counts and green/checkmarked selections |
...
The current 23-product catalog fits in one selection message, with no **Open product**, **Next** or **Previous** buttons. Full names appear in the wrapping message text and unshortened button payloads. Product numbers are persistent catalog IDs, so similar pack/flavor names can be matched even if a Telegram client visually clips a long button label. Telegram does not provide a button-wrapping control: the bot cannot force every client to display a long button label in full. Selected buttons use the native green `success` style plus a checkmark, and the text includes selected/unselected markers and a count, so color is not required.

If a future catalog exceeds Telegram's text limit or the conservative 100-button budget, every product is automatically queued in consecutive, rate-limited messages, without interactive paging or shortened names. Each chunk shows **its own** selected count (not a duplicated global total that could become stale); `/status` gives the current overall count. A toggle updates the tapped chunk after its selection is committed. Buttons carry both the current configuration revision and the stable product-ID range of their message. A stale button refreshes that range without applying an old intent; tap again to make the selection. Old already-sent paginated buttons upgrade to the full-catalog view without changing any selection.

Toggles use the cached catalog rather than fetching Amul again. While a large catalog is still arriving, a toggle reports that delivery is in progress and leaves selections unchanged; this prevents a new revision from discarding not-yet-delivered chunks. Products that disappear remain visible as tracked/UNKNOWN so they can still be unselected. Refresh `/products` after a PIN change before adding selections. Product links remain in restock alerts; only the selection-menu URL buttons were removed. These UI choices do not change quiet baselines, notification rules, or the configured polling schedule.
...
`npm test` executes in the local Workers runtime with a real local D1 binding, the production migration, non-secret fixtures and **mocked Amul/Telegram fetches**. It covers regional request signing/cookies/preferences, upstream pagination, quantity-vs-availability, silent baselines, restocks/re-arming, unknown/error preservation, PIN/watch changes, owner validation, stale callbacks, deduplication, outbox retry/rate limits, overlapping work, stale leases and transactional rollback. Selector tests include the full 23 public product names, green/checkmarked persisted toggles, no URL/navigation buttons, full Unicode names, byte/character limits, automatic overflow, message-specific updates and legacy-menu safety. Snapshot tests cover available-first grouping/counts, exact pack names, IST date conversion, safe HTML/Unicode, unknown/error cases and complete-name chunk boundaries. No real messages or Cloudflare resources are needed.
